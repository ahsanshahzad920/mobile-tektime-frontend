...
import { convertCount2ToSeconds } from "../../Meeting/GetMeeting/Helpers/functionHelper";
...
const KanbanCustomCard = ({
  ...
  return (
    ...
    >
      {/* Three-dot menu icon */}
      {step?.step_status !== "completed" && <div
        ref={iconRef}
...
const KanbanMissionBoard = ({ data, meeting, refreshMeeting, users }) => {
  ...
  useEffect(() => {
    const grouped = {
      planned: data.filter((s) => s.step_status === null),
      todo: data.filter(
  ...
  const handleClick = (item, index) => {
    if (item?.step_status === "in_progress") {
      navigate(`/actīon-play/${item?.meeting?.id}/${item?.id}`);
    } else {
      navigate(`/step/${item?.id}`, {
        state: { meeting: item?.meeting },
      });
    }
  };
  ...
  const handleForceStart = async (step) => {
    ...
    try {
      ...
      if (response?.status) {
        ...
        step?.meeting?.type === "Prestation Client"
        ) {
          markTodoMeeting(step?.meeting?.id);
        }

        navigate(`/actīon-play/${step?.meeting?.id}/${step?.id}`);
      }
    } catch (error) {
  ...
  const handleCopyStep = async (item) => {
    const maxOrderNo = Math.max(
      ...item?.meeting?.steps?.map((step) => step.order_no ?? 0),
      0
    );
...
export default KanbanMissionBoard;