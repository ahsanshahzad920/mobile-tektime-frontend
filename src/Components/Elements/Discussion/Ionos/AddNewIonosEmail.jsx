...
const AddNewIonosEmail = ({ show, onClose, onEmailAdded }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/ionos-emails/send-email`, {
        method: "POST",
        headers: { Authorization: `Bearer ${TOKEN}` },
        body: formData,
      });
  ...
  return (
    ...
    >
      ...
      >
        <SiIonos
          size={isMobile ? 28 : 36}
          style={{ color: "#003D8F" }} // Official Ionos Blue
        />
        <h2
          style={{
...
export default AddNewIonosEmail;
