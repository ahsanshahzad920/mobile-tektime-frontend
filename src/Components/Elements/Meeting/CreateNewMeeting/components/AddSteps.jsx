...
const AddStpes = ({ setActiveTab, meeting1 }) => {
  const {
    formState,
    setFormState,
    handleInputBlur,
  ...
  useEffect(() => {
    ...
    if (meeting?.steps && meeting?.steps.length > 0) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.status) {
        ...
        setMeeting({ ...meeting, steps: reorderedSteps });

        // ✅ Call reorder API only if meeting type is not 'Task' or 'Strategy'
  ...
  const [items, setItems] = useState(meeting?.steps); // Store the items in the state
  ...
  const onDragEnd = async (result) => {
    ...
    // //  Block if dragged step is Task or Strategy
    // if (meeting?.type === "Task" || meeting?.type === "Strategy") {
    //   console.log("Cannot drag steps of type Task or Strategy.");
    //   return;
    // }
  ...
  const handleOrderChange = async (step, newOrderNo) => {
    const existingStep = steps.find((s) => s.order_no === newOrderNo);
  ...
  const [isOrderUpdated, setIsOrderUpdate] = useState(false);
  ...
  const [editingCount2, setEditingCount2] = useState(null);

  // const handleIncrementCount2 = (stepId, currentCount) => {
  //   const step = steps.find((s) => s.id === stepId);
  //   if (step) {
  //     let newCount;

  //     if (meeting?.type === "Absence") {
  //       newCount = parseFloat(currentCount) + 0.5;
  //     } else {
  //       newCount = parseFloat(currentCount) + 1;
  //     }
  //     updateCount2(stepId, newCount);
  //   }
  // };

  // const handleDecrementCount2 = (stepId, currentCount) => {
  //   const step = steps.find((s) => s.id === stepId);
  //   if (step) {
  //     let newCount;
  //     if (meeting?.type === "Absence") {
  //       newCount = Math.max(parseFloat(currentCount) - 1, 0);
  //     } else {
  //       newCount = Math.max(parseFloat(currentCount) - 0.5, 0);
  //     }
  //     updateCount2(stepId, newCount);
  //   }
  // };

  // const handleCount2Change = (stepId, value) => {
  //   // Validate input for Absence type (allow numbers with .5)
  //   if (meeting?.type === "Absence") {
  //     if (/^\d*\.?5?$/.test(value) || value === "") {
  //       updateCount2(stepId, value);
  //     }
  //   } else {
  ...
  if (step) {
    ...
    const updatedSteps = steps.map((s) =>
      s.id === stepId ? { ...s, count2: newCount } : s
    );
    updateSteps(updatedSteps);
    setMeeting((prev) => ({ ...prev, steps: updatedSteps }));
    setFormState((prev) => ({ ...prev, steps: updatedSteps }));
...
const updateCount2 = async (stepId, newCount) => {
  try {
    ...
    const response = await axios.put(
      `${API_BASE_URL}/steps/${stepId}`,
      {
        ...
        time: newCount,
        _method: "put",
                  // google_agenda_update: stepToUpdate?.editor_type === "Google Meet Link" ? true : false
                   google_agenda_update: meeting?.type === "Google Agenda Event" || meeting?.agenda === "Google Agenda" ? true : false,
                   outlook_agenda_update: meeting?.type === "Outlook Agenda Event" || meeting?.agenda === "Outlook Agenda" ? true : false
    ...
    if (response.data.success) {
      ...
      const updatedSteps = steps.map((step) =>
        step.id === stepId ? { ...step, count2: newCount } : step
      );
      updateSteps(updatedSteps);
    ...
    console.error("Error updating count2:", error);
    
    // Check for specific validation errors
    if (error.response?.data?.errors?.count2) {
      toast.error(error.response.data.errors.count2[0]); // Show first validation error
    } 
    ...
    // Fallback to generic error
      ...
      toast.error("Failed to update time of step");
    }
  }
};
  ...
  const toggleAssigneeDropdown = (stepId) => {
    setActiveDropdown(activeDropdown === stepId ? null : stepId);
  };

 const handleAssigneeSelect = async (stepId, participant) => {
  try {
    const stepToUpdate = steps.find((step) => step.id === stepId);
    if (!stepToUpdate) return;
    ...
    const response = await axios.put(
      `${API_BASE_URL}/steps/${stepId}`,
      {
        ...
        assigned_to_name: `${participant.first_name} ${participant.last_name}`,
        assigned_to_image: participant.participant_image,
        _method: "put",
      },
      {
        headers: {
    ...
    if (response.data.success) {
      ...
      setFormState((prevMeeting) => ({
        ...prevMeeting,
        steps: updatedSteps,
      }));
      setActiveDropdown(null);
      toast.success(t("Assignee updated successfully"));
    }
  } catch (error) {
    ...
    <div className=" col-md-12 mt-1 modal-height">
        <Row className="mb-4">
          <Col xs={12} sm={12} md={12} lg={12}>
            ...
            <div
              className="d-flex justify-content-between align-items-center modal-tab-button px-3"
              // onClick={handleShow}
              onClick={() => stepModal()}
            >
        ...
        >
          {steps?.length > 0
            ? steps?.length + " " + t("meeting.formState.step.Steps added")
            : ""}
        </h4>
        <div className="table-responsive">
          <DragDropContext onDragEnd={onDragEnd}>
            <Droppable droppableId="steps">
              {(provided) => (
                <table
                  className="table add-guest-table addsteptable"
                  {...provided.droppableProps}
                  ref={provided.innerRef}
                  style={{
                    opacity: isDrag || isOrderUpdated ? 0.5 : 1,
                    pointerEvents: isDrag || isOrderUpdated ? "none" : "auto",
                  }}
                >
                  {loading ? (
                    <Spinner
                                ...
                                >
                                  {step?.assigned_team?.logo ? (
                                    <img
                                      ...
                                      alt={`${step?.assigned_team?.name}'s avatar`}
                                ...
                                >
                                  ...
                                  >
                                    <div
                                      ...
                                      onClick={() =>
                                        toggleAssigneeDropdown(step.id)
                                      }
                                      style={{ cursor: "pointer" }}
                                    >
                                      {step.assigned_to_image ? (
                                        <img
                                          src={
                                            step.assigned_to_image.startsWith(
                                              "users/"
                                            )
                                    ...
                                    {activeDropdown === step.id && (
                                      <div
                                        className="assignee-dropdown-list"
                                        style={{
                                          position: "absolute",
                                          top: "100%",
                                          left: "50%",
...
>
  {meeting?.type === "Sprint" ? (
    // 🔹 Dropdown only for Sprint
    <div className="d-flex align-items-center gap-2 justify-content-center">
      <select
        value={step.count2}
        onChange={(e) => handleCount2Change(step.id, e.target.value)}
    ...
    // 🔹 Original + / - / input for all other meeting types
    <div className="d-flex align-items-center gap-2 justify-content-center">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="17"
        ...
        viewBox="0 0 17 16"
        fill="none"
        onClick={() => handleDecrementCount2(step.id, step.count2)}
        style={{ cursor: "pointer" }}
      >
        <rect x="0.800049" width="16" height="16" rx="4" fill="white" />
        <path
      ...
      <svg
        ...
        viewBox="0 0 17 16"
        fill="none"
        onClick={() => handleIncrementCount2(step.id, step.count2)}
        style={{ cursor: "pointer" }}
      >
        <rect x="0.600098" width="16" height="16" rx="4" fill="white" />
        <path
      ...
      <div className="d-flex align-items-center gap-3">
        <h6 className="m-0">
          {t("meeting.newMeeting.labels.Estimated duration of the moment")}
        </h6>
        <span>
