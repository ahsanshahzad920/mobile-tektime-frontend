...
const ReportStepCard = ({
  ...
  const hasStepMedias =
    (item?.step_medias && item.step_medias.length > 0) ||
    item?.step_status === "completed" ||
    (stepMedias &&
      stepMedias.filter((media) => media.step_id === item.id).length > 0);

  const isClosed =
    item?.step_status === "closed" ||
    item?.step_status === "completed" ||
 ...
 const [dropdownVisible, setDropdownVisible] = useState(
  isClosed
    ? true   // 👈 pehle: hasStepMedias, ab hamesha true
    : !!(
        item.editor_content ||
        item.note ||
        item.editor_type === "File" ||
        item.editor_type === "Url" ||
        item?.editor_type === "Publication" ||
        item?.editor_type === "AI Instruction" ||
        hasStepMedias
      ),
);
      ...
      <Card.Body onClick={toggleDropdown}>
        <Row>
          ...
          <Col xs={12} md={dropdownVisible ? 12 : 8} className="step-details">
            ...
            <Card.Text
              className={`step-card-content d-flex ${meeting?.type === "Calendly" ? "flex-column align-items-start gap-1" : "align-items-center flex-wrap gap-2"}`}
            >
