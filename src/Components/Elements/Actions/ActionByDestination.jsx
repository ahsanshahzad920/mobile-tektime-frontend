...
import { Card, OverlayTrigger, Tooltip, ProgressBar } from "react-bootstrap";
...
function ActionByDestination({ stepsData, refreshTrigger }) {
  ...
  const getActions = async (calculation = false) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/get-destination-steps/${selectedTab?.id}?filter=${selectedTitle}&current_time=${formattedTime}&current_date=${formattedDate}&timezone=${userTimeZone}${calculation ? "&calculation=true" : ""
        }`
      );
      if (response.status) {
  ...
  const refreshedActions = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/get-destination-steps/${selectedTab?.id}?filter=${selectedTitle}&current_time=${formattedTime}&current_date=${formattedDate}&timezone=${userTimeZone}`
      );
      if (response.status) {
        const data = response?.data?.data || [];
        setSteps(data);
  ...
  const formattedWorkload =
    totalDays > 0
      ? `${totalDays} ${t("time_unit.day", { count: totalDays })} ${totalHours > 0
        ? `${totalHours} ${totalHours === 1 ? t("time_unit.hour") : t("time_unit.hours")
        }`
        : totalMinutes > 0
      ...
      : totalHours >= 1
        ...
        : `${totalMinutes} ${totalMinutes === 1 ? t("time_unit.minute") : t("time_unit.minutes")
        }`;
  ...
  const loggedInUserId = JSON.parse(CookieService.get("user")); // Get the logged-in user's ID from session storage
  const userMail = loggedInUserId?.email;
  ...
  return (
    <>
      <div className="action-by-destination-scroll-container ">
        {stepsData?.map((item) => (
          <div key={item?.id}>
            <button
              className={`tab ${selectedTab?.id === item.id ? "active" : ""}`}
              style={{
                padding: "11px 11px",
                whiteSpace: "nowrap",
                backgroundColor:
                  selectedTab?.id === item.id
                    ? "rgb(54, 155, 224)"
                    : "rgb(241 241 241)",
                color: selectedTab?.id === item.id ? "#FFF" : "#000",
                border: "none",
                cursor: "pointer",
                borderRadius: "13px",
                fontWeight: "bold",
      ...
      <div className="container-fluid invite">
        ...
        <div
          ref={stickyRef}
          className="action-tabs-sticky mt-2 overflow-x-auto bg-white"
          style={{
            position: "sticky",
            top: 0,
            zIndex: 1,
        ...
        >
          <div className="d-flex flex-nowrap gap-2 px-2" style={{ minWidth: "fit-content" }}>
            {[
              {
                label: t("badge.Planned"), count: upcomingCount, bgcolor: "rgba(47, 86, 187, 0.1019607843)",
                color: "#2f56bb",
              },
              { label: t("badge.Todo"), count: todoCount, bgcolor: "#6c757d", color: "white" },
              { label: t("badge.inprogress"), count: inProgressCount, bgcolor: "#f2db43", color: "white" },
              {
                label: t("badge.completed"), count: completedCount, bgcolor: "rgba(47, 187, 103, 0.1019607843)",
                color: "#2fa25d"
              },
            ].map((tab, idx) => (
              <h6
                                        ...
                                        <div className="mb-2">
                                          ...
                                          <div className="d-flex align-items-center flex-wrap">
                                            {item?.meeting?.moment_privacy ===
                                              ...
                                              />
                                            ) : item?.meeting?.moment_privacy === "participant only" ? (
                                              <Avatar.Group maxCount={5}>
                                                {item?.meeting?.participants?.map((item) => {
                                                  return (
                                                    <>
                                                      <AntdTooltip
                                            ...
                                            ) : item?.meeting?.moment_privacy === "tektime members" ? (
                                              <img
                                                src={
                                            ...
                                            <span
                                              className={`badge ms-2 ${item?.meeting
                                                  ?.moment_privacy ===
                                                  "private"
                                                  ? "solution-badge-red"
                                                  : item?.meeting
                                                    ...
                                                    : item?.meeting
                                                      ?.moment_privacy ===
                                                      "enterprise" || item?.meeting?.moment_privacy === "participant only" ||
                                                      item?.meeting?.moment_privacy === "tektime members"
                                                      ? "solution-badge-blue"
                                                      : item?.meeting
                                                        ?.moment_privacy ===
                                                ...
                                                : item?.meeting
                                                  ...
                                                  : item?.meeting
                                                    ...
                                                    : item?.meeting?.moment_privacy === "participant only"
                                                      ...
                                                      : item?.meeting?.moment_privacy === "tektime members"
                                                        ? t("solution.badge.membersOnly") : item?.meeting
                                                          ?.moment_privacy ===
                                                          "password"
...
export default ActionByDestination;
