...
const Options = ({ setActiveTab }) => {
  ...
  const handleShareToggle = () => {
    if (formState?.type === "Special") return;
  ...
  useEffect(() => {
    if (solution) {
      console.log(solution, "meeeeeeeee");
      const playbackValue = solution.playback || ""; // Get the playback value
      const isAutomatic = playbackValue === "automatic"; // Check if playback is automatic
      setFormState((prevState) => ({
        ...prevState,
        prise_de_notes:
          solution?.prise_de_notes || t("meeting.formState.Manual"),
        feedback: solution?.feedback || false,
        remainder: solution?.remainder || false,
        notification: solution?.notification || false,
        note_taker: solution?.note_taker || false,
        alarm: solution?.alarm || false,
        playback: playbackValue,
      ...
      if (solution?.share_by === "email") {
        setToggleStates((prev) => ({ ...prev, shareby: true }));
        setIsShareByToggle(true);
      }
    }
  ...
  //     prevState === "Manual" ? "Automatic" : "Manual"
  //   );
  //   setFormState((prevFormState) => ({
  //     ...prevFormState,
  //     prise_de_notes: noteTaking,
  //   }));
  // };
  // const toggleNoteTaking = () => {
  //   if(solution?.type === "Special") return
  //   setNoteTaking((prevState) => {
  ...
  return (
    <div className="col-md-12 p-1 mt-1 p-4 modal-height">
      <div className="modal-body">
        <Row className="mb-4 align-items-center g-4">
          {solution?.type !== "Newsletter" && (
            <>
              <Col xs={12} md={6}>
                <div
