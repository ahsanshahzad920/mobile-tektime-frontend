...
function ActionByTeams({ stepsData, teamName, refreshTrigger }) {
  ...
  const getActions = async (calculation = false) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/get-team-steps/${selectedTab?.id}?filter=${selectedTitle}&current_time=${formattedTime}&current_date=${formattedDate}&timezone=${userTimeZone}${calculation ? "&calculation=true" : ""
        }`
      );
      if (response.status) {
  ...
  const refreshedActions = async () => {
    ...
    try {
      ...
      const formattedDate = formatDate(timeInUserZone);
      const response = await axios.get(
        `${API_BASE_URL}/get-team-steps/${selectedTab?.id}?filter=${selectedTitle}&current_time=${formattedTime}&current_date=${formattedDate}&timezone=${userTimeZone}`
      );
      if (response.status) {
        const data = response?.data?.data || [];
        setSteps(data);
