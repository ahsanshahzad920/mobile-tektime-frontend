...
const AddSolutionTeam = ({ handleClose }) => {
    const {
        loading,
        formState,
        setFormState,
    ...
    const getSolution1 = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/solutions/${checkId}?current_time=${formattedTime}&current_date=${formattedDate}`, {
                headers: {
    ...
    const handleDelete = async (team) => {
        try {
            ...
            const response = await axios.delete(
                `${API_BASE_URL}/delete-solution-team`,
                {
                    headers: {
                        Authorization: `Bearer ${CookieService.get("token")}`,
                    },
    ...
    const addTeams = async () => {
        ...
        try {
            const response = await axios.post(
                `${API_BASE_URL}/add-solution-team-participants`,
                payload,
                {
                    headers: {
                        Authorization: `Bearer ${CookieService.get("token")}`,
                ...
                <Col md={12}>
                    <Accordion className="mt-2 mb-2">
                        <Accordion.Item eventKey="0">
                            <Accordion.Header
                                className={`custom-accordion-header ${expanded ? "expanded" : ""
                                    }`}
                                ...
                                <div className="list-group">
                                    {teams?.length > 0 ? (
                                        teams.map((team) => (
                                            <button
                                                key={team.id}
                                                className={`list-group-item list-group-item-action p-3 ${selectedTeams?.some((t) => t.id === team.id)
                                                    ? "border-primary"
                                                    : ""
                                                    }`}
...
export default AddSolutionTeam;
