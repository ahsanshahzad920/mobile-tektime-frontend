...
const CSVImportStepper = ({
  ...
  // Database fields we want to map to
const dbFields = [
  { id: "email", name: t("Email"), required: true, type: "email" },
  { id: "first_name", name: t("First Name"), required: false },
  { id: "last_name", name: t("Last Name"), required: false },
  { id: "phone_number", name: t("Phone"), type: "phone" },
  { id: "role", name: t("Role/Position") },
  ...(!fromClient ? [{ id: "client", name: t("Organization") }] : []),
];
  ...
  const validateData = () => {
    ...
    // Only validate required fields if they're mapped
    dbFields
      .filter((f) => f.required)
      .forEach((field) => {
    ...
    // Validate email format only for mapped emails
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      //   // Add enterprise_id to each record
      //   return {
      //     ...mappedData,
      //     _row: index + 1,
      //     enterprise_id: enterpriseId, // Add enterprise_id here
      //   };
      // });
      const payload = csvData.map((row, index) => {
        const mappedData = {};
        Object.entries(fieldMappings).forEach(([dbField, csvHeader]) => {
          // Skip client/organization field if fromClient is true
          if (dbField === "client" && fromClient) return;
        ...
        // Add enterprise_id and client_id (if fromClient) to each record
        const record = {
          ...mappedData,
          _row: index + 1,
      ...
      for (let i = 0; i < payload.length; i += batchSize) {
        ...
        await axios.post(`${API_BASE_URL}/add-bulk-contacts`, batch, {
          headers: {
      ...
      toast.success(`${payload.length} contacts ${t("processed")}`);
      refreshContacts();
      handleClose();
    } catch (error) {
 ...
 setIsUploading(false);
  ...
  if (apiErrors) {
    const formattedErrors = Object?.entries(apiErrors)
      .map(([field, messages]) => {
        // Extract row number and field name
        const [index, fieldName] = field.split(".");
        const msg = messages?.join(", ");
        return `Row ${parseInt(index) + 1}: ${fieldName} - ${msg}`;
      })
      .join("\n");

    toast.error(
      <div>
        <div>{apiMessage || "Import completed with some errors"}:</div>
        <pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{formattedErrors}</pre>
      </div>,
      { autoClose: false }
    );
  } else {
    toast.error(apiMessage || "Import completed with some errors");
  }
  }
  };
    ...
    >
      ...
      <div className="requirements mt-4">
        ...
        <ul>
          ...
          <li>
            {/* {t("Should include email, first name, and last name columns")} */}
            {t("Should include at least an email column")}
        ...
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation(); // This prevents the event from reaching parent elements
            const link = document.createElement("a");
            link.href = "/templates/contacts_template.csv";
    ...
    <div className="mapping-container">
      <div className="mapping-header">
        ...
        <p className="text-muted">
          {t("Match your CSV columns to the corresponding contact fields")}
        </p>
      </div>
  ...
  // Render data preview - MODIFIED TO SHOW ALL RECORDS REGARDLESS OF VALIDATION
  const renderDataPreview = () => {
    // Create preview data with all fields mapped
