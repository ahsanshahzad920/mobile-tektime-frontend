...
import { getAllIonosEmailDestinations, getIonosEmailMeetings } from "../api";
import useMeetingPolling from "../Hooks/useMeetingPolling";

const { Text } = Typography;
const { Content } = Layout;

const IonosTab = ({ searchTerm, isActive, userData, refreshTrigger, refreshUser, onShowCompose }) => {
  ...
  const fetchWithFolder = useCallback(
    (id, page) => getIonosEmailMeetings(id, page, folder, searchTerm),
    [folder, searchTerm],
  );
  ...
  const fetchDestinations = useCallback(async () => {
    try {
      const query = userData?.ionos_links?.[0]?.first_sync === 0 ? "first_sync=true" : "sync=true";
      const data = await getAllIonosEmailDestinations(query);
  ...
  useEffect(() => { if (isActive) refreshUser(); }, [isActive, refreshUser]);
