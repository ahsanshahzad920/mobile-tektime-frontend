...
import { Form, Button, Row, Col, ButtonGroup, Spinner, Modal } from "react-bootstrap";
...
function DateandTime({ setActiveTab }) {
  const {
    formState,
    setFormState,
    handleInputBlur,
  ...
  useEffect(() => {
    if (isCalendly) {
      setFormState((prev) => {
        // Always ensure timezone is set if it's missing or null
        const timezone = prev.calendly_timezone || "Europe/Paris";

        // Check if we need to set default availability
        const availability = (prev.calendly_availability && prev.calendly_availability.length > 0)
          ? prev.calendly_availability
          : [
            "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche",
          ].map((day, index) => ({ day, active: index < 5, start: "09:00", end: "17:00" }));

        // Only update if something changed
        if (prev.calendly_timezone !== timezone || prev.calendly_availability !== availability) {
          return {
            ...prev,
            calendly_timezone: timezone,
            calendly_availability: availability,
            calendly_non_availability: prev.calendly_non_availability || [],
          };
        }
        return prev;
      });
    }
  }, [isCalendly, setFormState]);
  ...
  useEffect(() => {
    ...
    // If there's no meeting data, set the selectedDateTime to current date
    if (!meeting) {
  ...
  useEffect(() => {
    if (meeting) {
      ...
      const translatedSelectedDays =
        language === "en"
          ? translateDaysToEnglish(meeting?.selected_days || [])
          : translateDaysToFrench(meeting?.selected_days || []);

      setFormState((prevState) => ({
        ...prevState,
        date: existingDateTime?.toISOString().split("T")[0] || null,
        start_time: existingDateTime?.toTimeString().split(" ")[0] || null,
        repetition: meeting.repetition || false,
        repetition_number: meeting.repetition_number || 1,
        repetition_frequency: meeting.repetition_frequency || "Daily",
        repetition_end_date: meeting.repetition_end_date || null,
        selected_days: meeting.selected_days,
      }));
      setSelectedDays(translatedSelectedDays || []); // Sync with selectedDays state
      setSelectedDateTime(existingDateTime);
      setShowRepeatSection(meeting?.repetition);
    }
  ...
  return (
    <div className="pt-3 modal-height">
      {!isCalendly && (
        <>
          <Row>
            <Col md={12}>
                    ...
                    options={{
                      ...
                      maxDate:
                        meeting?.type === "Special" || meeting?.type === "Law"
                          ? new Date()
                          : null, // Restrict future dates for "Special" && "Law" meetings
                    }}
                  />
                </div>
              </Form.Group>
                      ...
                      {dayConfig.active ? (
                        <div className="d-flex align-items-center gap-2">
                          <Form.Control
                            ...
                            onChange={(e) => {
                              const newAvailability = [...(formState.calendly_availability || [])];
                              const targetIndex = newAvailability.findIndex(
                                (d) => d.day === day
                              );
                              if (targetIndex >= 0) {
                                newAvailability[targetIndex].start = e.target.value;
                                setFormState({
                                  ...formState,
                          ...
                          <Form.Control
                            ...
                            onChange={(e) => {
                              ...
                              if (targetIndex >= 0) {
                                newAvailability[targetIndex].end = e.target.value;
                                setFormState({
                                  ...formState,
                                  calendly_availability: newAvailability,
                                });
