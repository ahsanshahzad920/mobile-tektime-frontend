...
const QuickStepChart = ({
  ...
  refreshMeeting,
  isCopied = false,
  createStepFromAction = false,
  closeMeeting,
  recurrentMeetingAPI,
}) => {
  const { checkId, setCall, formState } = useFormContext();
  ...
  const options = [
    ...
    // {
    //   value: "PowerPoint",
    //   label: t("stepModal.powerpoint"),
    //   icon: <PiMicrosoftPowerpointLogo className="fs-5" />,
    // },
    // {
    //   value: "Validation",
    //   label: t("stepModal.validation"),
    //   icon: <SiGooglemeet className="fs-5" />,
    // },
  ];
  ...
  const AbsenceOption = [
    ...
    {
      value: "Publication",
      label: t("stepModal.publication") || "Publication",
  ...
  console.log("participantLabels", participantLabels);
  ...
  }, [show, meeting, meeting?.id, storedStartTime, storedStartDate, selectedCount]);
  ...
  const handleIncrementCount = () => {
    ...
    if (meeting?.type === "Absence") {
      setSelectedCount((prev) => {
        const newValue = parseFloat(prev) + 0.5;
        return newValue % 1 === 0 ? newValue : newValue; // Keep as float
      });
    } else {
...
Ces jours peuvent être posés par le salarié, ou dans certains cas, imposés par l'employeur (RTT collectif).`,
  ...
  const validateStep = async () => {
    ...
    if (id == null) {
      ...
      // Step 1: Convert date from dd/mm/yyyy to yyyy-mm-dd
      const [day, month, year] = storedStartDate?.split("/");
      const stepDate = `${year}-${month}-${day}`;

      // Step 2: Convert time from 15h50 to 15:50
      const stepTime = storedStartTime?.replace("h", ":");
      const payload = {
        order_no: selectedOrder || 1,
        title: selectedValue,
    ...
    if (createStepFromAction) {
      window.location.reload();
    }
    // Call refreshMeeting only if it exists
    ...
    if (id == null) {
      ...
      const payload = {
        ...
        editor_content:
          ...
          modalType === "Story" ||
          modalType === "Question" ||
          modalType === "Email" || modalType === "Publication" || modalType === "AI Instruction"
            ? optimizedEditorContent || ""
            : [
                  "Absence CET non payable",
                  "Absence CET payable",
      ...
      try {
        ...
        setProgress(0);
        setIsValidate(false);
        const errMsg = error?.response?.data?.errors?.title?.[0] || error?.response?.data?.message || error?.message;
        toast.error(t(`meeting.newMeeting.${errMsg}`) || errMsg);
      } finally {
        setIsQuestionProcessing(false);
        setQuestionFinalizing(false);
      }
...
const getStep = async () => {
    ...
    if (id) {
      console.log("eidt case");
      getStep();
    }
  }, [id]);

  // Helper: Add duration to datetime
  function addDurationToDateTime(date, time, count, unit, timezone) {
    ...
    // Normalize unit for moment.js
    let momentUnit = "minutes";
    if (unit === "day" || unit === "days") momentUnit = "days";
    else if (unit === "hour" || unit === "hours") momentUnit = "hours";
    else if (unit === "second" || unit === "seconds") momentUnit = "seconds";

    const updated = dateTime.add(count, momentUnit);

    return {
      formattedDate: updated.format("YYYY-MM-DD"),
      formattedTime: updated.format("HH:mm:ss"),
    };
  }

  useEffect(() => {
    // Helper: Format datetime based on timezone and timeUnit
    const formatDateTime = (date, time, timezone, timeUnit) => {
      ...
      return {
        ...
        formattedDate: dateTimeInUserTZ.format("DD/MM/YYYY"),
      };
    };
    ...
    if (id && step) {
      ...
      if (stepTime && stepDate) {
        ...
        setStoredStartTime(result.formattedTime);
        setStoredStartDate(result.formattedDate);
        setStoredStartDateForHour(result.formattedDate);
      }
    } else if (meeting?.steps?.length > 0) {
      // Case 2: Adding new step, and steps exist
      ...
      if (
        ...
        const nextStepDateTime = addDurationToDateTime(
          lastStep.start_date,
          lastStep.step_time,
          lastStep.count2,
          lastStep.time_unit,
        ...
        const result = formatDateTime(
          nextStepDateTime.formattedDate,
          nextStepDateTime.formattedTime,
          meetingTimezone,
          lastStep.time_unit,
        );
      ...
      // Case 3: No steps, use meeting start
      const stepTime = meeting?.start_time;
      const stepDate = meeting?.date;

      if (stepTime && stepDate) {
        const result = formatDateTime(stepDate, stepTime, meetingTimezone);
        setStoredStartTime(result.formattedTime);
        setStoredStartDate(result.formattedDate);
        setStoredStartDateForHour(result.formattedDate);
      }
    }
  }, [step, id, meeting?.steps]);
  useEffect(() => {
    if (id || step) {
  ...
  }, [meeting?.steps, step, id]); // Update when steps or editing step changes
  ...
  const deleteStep = async () => {
    try {
      ...
      if (response.data) {
        ...
        if (meeting?.type !== "Task" && meeting?.type !== "Strategy") {
          try {
            await axios.post(
  ...
  //     }
  //   }
  // }, [link]); // Runs when the `link` is set
  ...
  const addItem = () => {
    if (newItem.trim() !== "") {
      setItems([
        ...items,
        { id: Date.now(), text: newItem, completed: false, pinned: false },
      ]);
      setNewItem("");
    }
  };
        ...
        <div id="chart-container" className="chart-content">
          ...
          >
            ...
            >
              ...
              >
                <div className="col-12 col-lg-3" style={{ borderBottom: isMobileView ? "1px solid #EAECF0" : "none" }}>
                  <div
                    className="input-field step-inputfield"
                    style={{
                  ...
                  >
                    ...
                    meeting?.type === "Prestation Client" ? (
                      ...
                      >
                        {Array.from(
                          { length: meeting?.steps?.length + 1 || 1 },
                          (_, i) => i + 1,
                        ).map((order) => (
                    ...
                    {!window.location.href.includes("/meetingDetail") && (
                      <div className="p-0 timecard" style={{ marginBottom: "4px" }}>
                        ...
                        <div className="d-flex align-items-center gap-2">
                          ...
                          />
                          <span className="step-time mb-0" style={{ fontSize: "12px", color: "#344054", fontWeight: "500", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "80px" }}>
                            {creator?.full_name}
                          </span>
                        </div>
                      </div>
                          ...
                          <div className="Editor-custom-dropdown" style={{ width: "100%", position: "relative" }}>
                            {meeting?.type === "Newsletter" || meeting?.type === "Social Media Newsletter" || meeting?.type === "AI Social Media Newsletter" ? (
                              <>
                                ...
                                <span className="step-time m-0" style={{ fontSize: "12px", color: "#344054", fontWeight: "600" }}>
                              ...
                              <>
                                      ...
                                      {user ? (
                                        ...
                                        <img
                                          src={
                                            creator?.image?.startsWith("users/")
                                              ? Assets_URL + "/" + creator?.image
                                              : creator?.image || "https://mdbcdn.b-cdn.net/img/new/avatars/2.webp"
                                          }
                                          className="rounded-circle"
                                          style={{
                                            borderRadius: "50%",
                                ...
                                {StepPresentor && (
                                  ...
                                  >
                                    {meeting?.user_with_participants?.length === 0 ? (
                                        ...
                                        {meeting?.user_with_participants
                                          ?.reduce((uniqueParticipants, item) => {
                                            const isDuplicate = uniqueParticipants.some(
                                              (participant) =>
                                                participant.first_name === item.first_name &&
                                                participant.last_name === item.last_name &&
                                          ...
                                          .map((item, index) => {
                                            return item.first_name === null &&
                                              ...
                                              <li
                                                ...
                                                disabled={window.location.href.includes("/meetingDetail") || fromReport}
            ...
            >
              <div className="p-0 timecard stepinfo" style={{ marginLeft: 0, width: "100%" }}>
                {!window.location.href.includes("/meetingDetail") && (
                  <div className="p-0 timecard" style={{ width: "100%" }}>
                    <p className="mb-1" style={{ fontSize: "10px", color: "#667085", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                      {t("meeting.newMeeting.Step format planify")}
                    </p>
                    <div className="d-flex justify-content-start" style={{ width: "100%" }}>
                      <div className="Editor-custom-dropdown" style={{ width: "100%", position: "relative" }}>
                        ...
                        >
                          <span style={{ fontSize: "12px", color: "#344054", fontWeight: "500", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                {modalType === "Editeur"
                                  ? t("stepModal.editor")
                                  : modalType === "File"
                                    ? t("stepModal.pdf")
                                  ...
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                    ...
                                    {/* <div>
                                      ...
                                      <Button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          document.getElementById("add-extra-media-quick").click();
                                        }}
              ...
              <div
                className="modal-footer-step step-chart-modal-footer align-items-center"
                style={{
                  position: "sticky",
                  bottom: 0,
                  left: 0,
