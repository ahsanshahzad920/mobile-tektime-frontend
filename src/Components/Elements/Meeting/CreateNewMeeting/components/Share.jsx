...
const Share = ({ setActiveTab }) => {
  ...
  const { formState, setFormState, handleInputBlur, meeting,isUpdated,handleCloseModal ,recurrentMeetingAPI,checkId} =
    useFormContext();
  ...
  useEffect(() => {
    if (meeting) {
      setFormState((prevState) => ({
        ...prevState,
        share_by: meeting?.share_by || "",
      }));

      if (meeting?.share_by === singleOption.value) {
        setSelectedCard(0);
      }
    }
