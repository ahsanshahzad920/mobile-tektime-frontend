...
import { getAllEmailDestinations, getDestinationEmailMeetings } from "../api";
import useMeetingPolling from "../Hooks/useMeetingPolling";

const { Text } = Typography;
const { Content } = Layout;

const OutlookTab = ({ searchTerm, isActive, userData, refreshTrigger, onShowCompose }) => {
  ...
  const fetchWithFolder = useCallback(
    (id, page) => getDestinationEmailMeetings(id, page, folder, searchTerm),
    [folder, searchTerm],
  );
  ...
  const fetchDestinations = useCallback(async () => {
    try {
      const data = await getAllEmailDestinations();
