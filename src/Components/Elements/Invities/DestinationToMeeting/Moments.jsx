...
import { FaLink, FaList } from "react-icons/fa6";
import { LuLink } from "react-icons/lu";
import copy from "copy-to-clipboard";
import { openLinkInNewTab } from "../../../Utils/openLinkInNewTab";
import { MdAssessment, MdContentCopy, MdInsertLink } from "react-icons/md";
import {
  abortMeetingTime,
...
const Moments = ({
  ...
  const updateMeetingStatus = async (e) => {
    ...
    try {
      const postData = {
        ...
        // step_decisions: null,
        step_notes: null,
        steps: item?.meeting_steps?.map((step) => ({
          ...step,
  ...
  const handleCopy = async (item) => {
    try {
      ...
      const updatedSteps = item?.meeting_steps?.map((step) => ({
        ...step,
        end_time: null,
        current_time: null,
        current_date: null,
  ...
  const continueHandlePlay = async (item) => {
    ...
    const updatedSteps = item?.meeting_steps?.map((step, index) => {
      if (index === 0) {
        return {
          ...step,
          status: "in_progress",
  ...
  const continueChangeStatusAndPlay = async (item) => {
    ...
    // const inProgressStep = item?.meeting_steps?.find(
    //   (step) => step?.step_status === "in_progress"
    // );
    // // Find the step with step_status "in_progress"
    // for (const step of item?.meeting_steps || []) {
  ...
  const formatDate = (item) => {
    if (item?.status === "no_status" && (!item?.date || !item?.start_time)) {
      return (
        <div
          style={{
            color: "#8590A3",
  ...
  const calculateEndTime = (start, steps) => {
    ...
    const endTime =
      hasSeconds && formattedEndSeconds !== 0
  ...
  sortedMeetings?.sort((a, b) => moment(a.date).diff(moment(b.date)));

  sortedMeetings
    // ?.filter((meeting) => meeting.status !== "draft")
    ?.forEach((item, index) => {
      const startDate = new Date(item?.date);
      const totalDays = calculateTotalDays(item?.meeting_steps);
      const endDate = addDaysToDate(startDate, totalDays);
      const formattedEndDate = formatDateInFrench(endDate);
      let date1;
      if (language === "en") {
        date1 = moment(item?.date).locale("en-gb"); // Set locale to English
      } else {
        date1 = moment(item?.date); // Set into french
      }
      const month1 = moment(date1).format("MMMM YYYY");
      if (!meetingsByMonth[month1]) {
        meetingsByMonth[month1] = [];
      }
      ...
      const shouldDisableButtons =
        ...
        item?.meeting_steps?.some((step) => step?.participant || step?.userPID);
      ...
      const startTimeDate = item?.start_time
      ...
      // Get individual components of the date
      const startDay = String(startDate1.getDate()).padStart(2, "0");
      ...
      const convertTimeTakenToSeconds = (timeTaken) => {
      ...
      const data = item?.meeting_steps?.forEach((step) => {
        const stepSeconds = convertTimeTakenToSeconds(step?.time_taken);
        totalSeconds += stepSeconds;
      });

      const endDate1 = new Date(startDate.getTime() + totalSeconds * 1000);

      // Get individual components of the date
      const endDay = String(endDate1.getDate()).padStart(2, "0");
      const endMonth = String(endDate1.getMonth() + 1).padStart(2, "0"); // Months are zero-based
      const endYear = endDate1.getFullYear();

      // Format the date in dd:mm:yyyy
      const formattedEndEstimateDate = `${endDay}/${endMonth}/${endYear}`;
      ...
      const realEndTime = calculateRealEndTime(item?.meeting_steps);
      ...
      //   return realEndTime;
      // };
      // const realEndTimeForClosed = calculateRealEndTimeForClosed(item?.meeting_steps);
      ...
      meetingsByMonth[month1].push(
        ...
        >
          <div className="row">
            <div
              className="col-lg-1 col-md-2 column-1"
              style={{ fontSize: "24px" }}
            >
              {formatDate(item)}
            </div>
            <div className="col-lg-10 col-md-9" style={{ paddingLeft: "18px" }}>
              <div className="row">
                <div className="col-md-12">
                  ...
                  {/* {item?.status === "draft" && (
                    <span className="mx-2 badge status-badge-green p-0">
                      {t("badge.draft")}
                    </span>
                  )} */}
                  {item?.status === "abort" && (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="time">{`(${
                            ...
                            item?.meeting_feedbacks?.length > 1
                              ? "Votes"
                              : "Vote"
                          })`}</span>
                        </div>
              ...
              {/* <div className="row mt-3">
                ...
                {guideCount > 0 && (
                  <div className="col-md-6 creator">{t("presentor")}</div>
                )}
              </div> */}
                      ...
                      >
                        {
                          ...
                          item?.meeting_steps?.some(
                            (guide) =>
                              guide?.userPID === parseInt(loggedInUserId),
                          ) ||
                          // Check if all participants have user_id equal to meeting.user.id
                          item?.user?.id === parseInt(loggedInUserId) ? (
                            <>
                              ...
                              {item?.status === "in_progress" ? (
                                <li>
                                  ...
                                  >
                                    <ImCancelCircle size={"20px"} color="red" />
                                    &nbsp; {t("dropdown.Cancel")}
                                  </a>
                                </li>
                              ) : (
