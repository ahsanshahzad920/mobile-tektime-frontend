        ...
        try {
          const assignedToUser =
            ...
            (
              ...
              []
            )?.find((participant) => participant.email === meeting?.user?.email)
              ?.id;
 ...
 const handleIncrementCount = () => {
  ...
  const validateStep = async () => {
    ...
    if (id) {
      ...
      const existingInProgressStepForAssignee = meeting?.steps?.find(
        (step) =>
          step.step_status === "in_progress" &&
          step.assigned_to === assignedToUser && // New assignee should not already have an in-progress step
          step.step_id !== id, // Exclude the current step being reassigned
      );
      ...
      const stepStatus = isSelfAssign
        ? meeting?.type === "Task" || meeting?.type === "Strategy"
          ? existingInProgressStepForAssignee
            ? "todo"
            : "in_progress"
          : "in_progress"
        : "to_accept";
      ...
      const payload = {
        ...
        assigned_to_team: meeting?.type === "Newsletter" ? null : null,
        status: stepStatus,
        step_status: stepStatus, // <-- Updated based on the condition
      ...
      try {
        ...
        const payload = {
          ...
          status: isSelfAssign ? null : "to_accept",
          url: modalType === "Url" ? (link ? link : null) : null,
          // order_no: stepOrder,
          meeting_id: meetingId,
          _method: "put",
  ...
  //FOR REPRENDE STEP
  const validateReprendeStep = async () => {
    // Update Excel file and get the updated file path
    let updatedExcelFileUrl = null;
    let updatedAudioReport = null;
    if (modalType === "Excel") {
      ...
      file:
        modalType === "File"
          ...
          : modalType === "Excel"
            ? updatedExcelFileUrl || fileName
            : fileName,
      assigned_to: meeting?.type === "Newsletter" ? null : assignedToYou,
      assigned_to_team: meeting?.type === "Newsletter" ? null : null,
      status: "in_progress",
      step_status: "in_progress",
      current_time: localCurrentTime,
          ...
          if (stepData?.prompt_ai) {
            try {
              ...
              if (parsed) {
                ...
                setAiPowerPointTemplate(parsed.aiPowerPointTemplate || null);
                setAiPdfTemplate(parsed.aiPdfTemplate || null);
              }
            } catch (e) {
...
export default StepChartAction;
