...
const MemberDetail = () => {
  ...
  const getMember = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/user-meetings/${id}?page=1`,
        {
          headers: {
  ...
  const loadMoreMeetings = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/user-meetings/${id}?page=${nextPage}`,
        {
          headers: {
            Authorization: `Bearer ${CookieService.get("token")}`,
          },
  ...
  const handleGenerateLink = async () => {
    try {
      ...
      toast.error("Error while sending mail. Please try again.");
    }
  };

     const handleVerfifyUser = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/admin/users/${id}/verify-email`,
        {
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
      ...
      // console.error("Error updating User:", error);
      toast.error(error?.response?.data?.message || "Error while verify user email. Please try again.");
    }
  };
          ...
          <div className={`tabs-container py-1 px-3`}>
            <div className="tabs-header">
              ...
              >
                ...
                <div
                  className={`col-lg-1 col-md-2 col d-flex justify-content-end p-0 `}
                >
                  {activeTab === "tab1" && member?.status === "pending" && (
                   <div className="d-flex gap-2">
                 {member?.email_verified_at === null && user?.role?.name === "MasterAdmin" &&   <button
                      className={`btn moment-btn`}
                      style={{ whiteSpace: "nowrap" }}
                      onClick={handleVerfifyUser}
                    >
                      {t("user.Verify user account")}
                    </button>}
                    <button
                      className={`btn moment-btn`}
                      style={{ whiteSpace: "nowrap" }}
