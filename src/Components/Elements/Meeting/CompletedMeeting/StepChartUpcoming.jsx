...
const StepChartUpcoming = ({
  ...
  const options = [
    ...
    //       label: t("stepModal.googleMeetLink"),
    //       icon: <SiGooglemeet className="fs-5" />,
    //     },
  ];
  ...
  const AbsenceOption = [
    {
      value: "Absence CET non payable",
      label: t("meeting.absence_step_type.absence_type_1"),
      icon: <DocumentIcon />,
    },
    {
      value: "Absence CET payable",
      label: t("meeting.absence_step_type.absence_type_2"),
      icon: <DocumentIcon />,
    },
    {
      value: "Partial unemployment",
      label: t("meeting.absence_step_type.absence_type_3"),
      icon: <DocumentIcon />,
    },
    {
      value: "Paid leave",
      label: t("meeting.absence_step_type.absence_type_4"),
      icon: <DocumentIcon />,
    },
    // {
    //   value: "Corporate event",
    //   label: t("meeting.absence_step_type.absence_type_5"),
    //   icon: <DocumentIcon />,
    // },
    // {
    //   value: "Skills sponsorship",
    //   label: t("meeting.absence_step_type.absence_type_6"),
    //   icon: <DocumentIcon />,
    // },
    {
      value: "Recovery",
      label: t("meeting.absence_step_type.absence_type_7"),
      icon: <DocumentIcon />,
    },
    {
      value: "RTT",
      label: t("meeting.absence_step_type.absence_type_8"),
      icon: <DocumentIcon />,
    },
  ];
  ...
  const confirmDeleteMedia = async (mediaId) => {
  ...
  useEffect(() => {
    const fetchParticipantLabel = async () => {
      if (!show || !meeting?.id || !storedStartDate || !storedStartTime) return;
      // Convert "01/07/2025" → "2025-07-01"
      const formattedDate = moment(storedStartDate, "DD/MM/YYYY").format(
        "YYYY-MM-DD",
      );

      // Convert "07h36" → "07:36:00"
      const formattedTime = storedStartTime?.replace("h", ":") + ":00";
      ...
      try {
        ...
        const response = await axios.post(
          `${API_BASE_URL}/get-participant-label`,
          payload,
        );

        console.log("API response:", response.data);
        setParticipantLabels(response.data.participant || []);
  ...
  }, [show, meeting, meeting?.id, storedStartTime, selectedCount]);
  const getLabelForParticipant = (email) => {
    // const { t } = useTranslation();
    const match =
      participantLabels.find((p) => p.email === email)?.label || null;

    // Map API labels to your translation keys if needed
    if (match === "Disponible") return t("statusLabel.available");
    if (match === "Occupé") return t("statusLabel.occupied");
    if (match === "Absent") return t("statusLabel.absent");
  ...
  const getStatusLabel = (status) => {
    ...
    switch (status) {
      case t("statusLabel.available"):
        color = "#2e7d32";
        bg = "#e8f5e9";
        break;
      case t("statusLabel.occupied"):
        color = "#ef6c00";
        bg = "#fff3e0";
        break;
      case t("statusLabel.absent"):
        color = "#c62828";
        bg = "#ffebee";
        break;
      default:
  ...
  }, [meeting?.type, meeting?.location, step?.location, show]);
    ...
    if (files.length > 0) {
      ...
      if (id === null) {
        ...
        try {
          ...
          const response = await axios.post(`${API_BASE_URL}/steps`, formData, {
            headers: {
          ...
          if (id === null) {
            ...
            try {
              const assignedToUser =
                user?.id ||
                (
                  meeting?.user_with_participants || meeting?.participants
                )?.find(
                  (participant) => participant.email === meeting?.user?.email,
                )?.id;
                  ...
                  modalType === "File"
                    ...
                    : modalType === "Video"
                      ...
                      : modalType === "Photo"
                        ...
                        : modalType,
                file: file,
                editor_content: (modalType === "Publication" || modalType === "AI Instruction" || meeting?.type === "AI Social Media Newsletter") ? modifiedFileText || null : null,
                meeting_id: meetingId,
                assigned_to: assignedToUser,
                // assigned_to_team: meeting?.type === "Newsletter" ? team : null,
                assigned_to_team: meeting?.type === "Newsletter" ? null : null,
  ...
  const handleIncrementCount = () => {
    ...
    if (meeting?.type === "Absence") {
      ...
      // Default increment by 1 for other types
      setSelectedCount((prev) => (Number(prev) || 0) + 1);
    }
  };

  const handleDecrementCount = () => {
    // For Absence type, decrement by 0.5 but don't go below 0
    if (meeting?.type === "Absence") {
      setSelectedCount((prev) => {
        const val = parseFloat(prev) || 0;
      ...
      // Default decrement by 1 for other types (with minimum 0)
  ...
  // Add state to store original order_no
  const [originalOrderNo, setOriginalOrderNo] = useState(null);

  // When editing a step, set the original order_no
  useEffect(() => {
    if (id) {
      // Fetch step data or set from props
      setOriginalOrderNo(step?.order_no); // Store the original value
    }
  ...
  const validateStep = async () => {
    ...
    if (!id) {
      ...
      const assignedToYou = (
        meeting?.user_with_participants || meeting?.participants
      )?.find((participant) => participant.email === SessionUser?.email)?.id;
      ...
      const payload = {
        ...
        editor_content:
          ...
          modalType === "Story" ||
          modalType === "Question" ||
          modalType === "Prestation" || modalType === "Email" || modalType === "Publication" || modalType === "AI Instruction" || meeting?.type === "AI Social Media Newsletter" || meeting?.type === "AI Instruction" ||
          modalType === "Message"
        ...
        file:
          modalType === "File"
            ? fileName
              ...
              : null
            : modalType === "Excel"
              ? updatedExcelFileUrl || (Array.isArray(fileName) ? JSON.stringify(fileName) : fileName)
              : Array.isArray(fileName) ? JSON.stringify(fileName) : fileName,
        assigned_to: meeting?.type === "Newsletter" ? null : assignedToUser,
        assigned_to_team: meeting?.type === "Newsletter" ? null : null,
        status: "active",
        step_status: null, // <-- Updated based on the condition
      ...
      try {
        ...
        if (!createAnother) {
          ...
          } else if (meeting?.type === "Law") {
            setModalType("Question");
          } else if (meeting?.type === "Absence") {
            setModalType("Absence CET non payable");
          } else if (meeting?.type === "Prestation Client") {
            setModalType("Prestation");
          } else if (meeting?.type === "Sprint") {
            setModalType("Story");
          } else if (meeting?.type === "Task") {
      ...
      try {
        ...
        const isAiFormUpdated =
          (meeting?.type === "AI Social Media Newsletter" || meeting?.type === "AI Instruction" || modalType === "AI Instruction") &&
          (normalize(aiSourceType) !== normalize(origPrompt.aiSourceType || "Text") ||
            normalize(aiSourceType === "Mission" ? selectedMission : aiSourceType === "Moment" ? selectedMoment : aiSourceText) !== normalize(origPrompt.aiSourceText || "") ||
        ...
        const payload = {
          ...
          file:
            modalType === "File"
              ...
              : modalType === "Excel"
                ? updatedExcelFileUrl || (Array.isArray(fileName) ? JSON.stringify(fileName) : fileName)
                : Array.isArray(fileName) ? JSON.stringify(fileName) : fileName, // assigned_to: assignedToUser,
          assigned_to: meeting?.type === "Newsletter" ? null : assignedToUser,
          assigned_to_team: meeting?.type === "Newsletter" ? null : null,
          ...
          // step_status: meeting?.type === "Special" ? "completed" : null,
          end_date: meeting?.type === "Special" ? formattedEndDate : null,
          // google_agenda_update: modalType === "Google Meet Link"? true : false
          google_agenda_update:
            meeting?.type === "Google Agenda Event" ||
            meeting?.agenda === "Google Agenda"
              ? true
              : false,
          outlook_agenda_update:
          ...
          try {
            ...
            const payload = {
              editor_content:
                ...
                modalType === "Story" ||
                modalType === "Question" ||
                modalType === "Prestation" || modalType === "Email" || modalType === "Publication" || modalType === "AI Instruction" || meeting?.type === "AI Social Media Newsletter" ||
                modalType === "Message"
                  ? optimizedEditorContent || ""
                  : [
                        "Absence CET non payable",
            ...
            const response = await axios.post(
              `${API_BASE_URL}/autosave-step-content/${id}?latest_current_time=${formattedTime}&latest_current_date=${formattedDate}`,
              payload,
              {
                headers: {
                  "Content-Type": "application/json",
            ...
            toast.error(
              t(
                `meeting.newMeeting.${error?.response?.data?.errors?.title[0]}`,
              ),
            );
          } finally {
            setIsValidate(false);
  ...
  const validateQuestionStep = async () => {
    ...
    if (!createAnother) {
      ...
      if (meeting?.type === "Newsletter") {
        ...
        setModalType("Absence CET non payable");
      } else if(meeting?.type === "Social Media Newsleter"){
        setModalType("Publication")
      } else {
        setModalType("Editeur");
      }
...
const getStep = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(
          `${API_BASE_URL}/steps/${id}?current_time=${formattedTime}&current_date=${formattedDate}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${CookieService.get("token")}`,
        ...
        if (response.status === 200) {
          ...
          setSelectedOrder(stepData?.order_no);

          setStepCreator(
            stepData?.step_creator?.full_name
              ? stepData?.step_creator?.full_name
              : stepData?.assigned_to_name,
          );
          setStepCreatorImg(
            stepData?.step_creator?.image
              ? stepData?.step_creator?.image
          ...
          if (stepData?.prompt_ai) {
            if (typeof stepData.prompt_ai === "object") {
              parsedPrompt = stepData.prompt_ai;
            } else if (typeof stepData.prompt_ai === "string") {
              try {
                parsedPrompt = JSON.parse(stepData.prompt_ai);
              } catch (e) {
                console.error("Failed to parse prompt_ai:", e);
              }
            }
          }

          if (parsedPrompt && typeof parsedPrompt === "object") {
            setAiSourceType(parsedPrompt.aiSourceType || "Text");
            if (parsedPrompt.aiSourceType === "Mission") {
              setSelectedMission( stepData?.source_id || parsedPrompt.sourceId || "");
              fetchMissions();
            } else if (parsedPrompt.aiSourceType === "Moment" || parsedPrompt.aiSourceType === "Discussion") {
              setSelectedMoment(stepData?.source_id || parsedPrompt.sourceId || "");
              fetchMoments();
            } else {
              setAiSourceText(parsedPrompt?.aiSourceText || "");

            }
            setAiSourceFile(parsedPrompt?.aiSourceFile || null);
            setAiRole(parsedPrompt?.aiRole || "");
            setAiAction(parsedPrompt?.aiAction || "");
            setAiObjective(parsedPrompt?.aiObjective || "");
            setAiRules(parsedPrompt?.aiRules || "");
            setAiOutputFormat(parsedPrompt?.aiOutputFormat || "Text");
            setAiOutputText(parsedPrompt?.aiOutputText || "");
  ...
  useEffect(() => {
    if (id) {
      ...
      const stepDate = step?.start_date;
      const meetingTimezone = meeting?.timezone || "Europe/Paris";

      if (stepTime && stepDate && meetingTimezone) {
        const userTimezone = moment.tz.guess();

        // Convert date and time to user's timezone
        const dateTimeInMeetingTZ = moment.tz(
          `${stepDate} ${stepTime}`,
          "YYYY-MM-DD hh:mm:ss A",
          meetingTimezone,
        );
        const dateTimeInUserTZ = dateTimeInMeetingTZ.clone().tz(userTimezone);

        // const timeParts = stepTime.split(" "); // Split by space to separate time and AM/PM
        // if (timeParts.length < 2) return; // Guard condition

        // const [hour, minute, second] = timeParts[0].split(":"); // Get hour, minute, second
        // const amPm = timeParts[1];

        // // Convert to 24-hour format safely
        ...
        const formattedTime =
          step?.time_unit === "seconds"
            ? dateTimeInUserTZ.format("HH[h]mm[m]ss")
            : dateTimeInUserTZ.format("HH[h]mm");

        setStoredStartTime(formattedTime);

        // Format date as DD/MM/YYYY
        const formattedDate = dateTimeInUserTZ.format("DD/MM/YYYY");
        // const formattedDate = moment(stepDate, "YYYY-MM-DD").format(
        //   "DD/MM/YYYY"
        // );
        setStoredStartDate(formattedDate);
        setStoredStartDateForHour(formattedDate);
      }
    } else if (meeting?.steps?.length > 0) {
      const lastStep = meeting.steps[meeting.steps.length - 1];
      const lastStepTime = lastStep?.step_time;
      const lastStepDate = lastStep?.start_date;
      const count2 = lastStep?.count2 || 0;
      const timeUnit = lastStep?.time_unit;
      const meetingTimezone = meeting?.timezone || "Europe/Paris";

      if (!lastStepTime || !lastStepDate) return; // Ensure time exists

      const userTimezone = moment.tz.guess();

      // Convert date and time to user's timezone
      const dateTimeInMeetingTZ = moment.tz(
        `${lastStepDate} ${lastStepTime}`,
        "YYYY-MM-DD hh:mm:ss A",
        meetingTimezone,
      );
      const dateTimeInUserTZ = dateTimeInMeetingTZ.clone().tz(userTimezone);

      // const timeParts = lastStepTime.split(" ");
      ...
      // let secondInt = parseInt(second, 10) || 0;
      ...
      // Create a moment object with the extracted time
      // let stepMoment = moment()
      //   .hours(hourInt)
      //   .minutes(minuteInt)
      //   .seconds(secondInt);

      // Add `count2` based on `time_unit` in user timezone
      if (timeUnit === "seconds") {
        dateTimeInUserTZ.add(count2, "seconds");
      } else if (timeUnit === "minutes") {
        dateTimeInUserTZ.add(count2, "minutes");
      } else if (timeUnit === "hours") {
        dateTimeInUserTZ.add(count2, "hours");
      } else if (timeUnit === "days") {
        dateTimeInUserTZ.add(count2, "days");
      }

      // Format the updated time in 24-hour format
      const formattedTime =
        timeUnit === "seconds"
          ? dateTimeInUserTZ.format("HH[h]mm[m]ss")
          : dateTimeInUserTZ.format("HH[h]mm");
      ...
      // Convert lastStepDate and add count2 to it in user's timezone
      const formattedDate = dateTimeInUserTZ.format("DD/MM/YYYY");

      setStoredStartDate(formattedDate);
      setStoredStartDateForHour(formattedDate);
    } else {
      const stepTime = meeting?.start_time;
      const stepDate = meeting?.date;
      const meetingTimezone = meeting?.timezone || "Europe/Paris";

      if (!stepTime || !stepDate) return; // Guard condition
      const userTimezone = moment.tz.guess();
      ...
      // const [hour, minute, second] = stepTime.split(":"); // Extract hour, minute, second
  ...
  const deleteStep = async () => {
    try {
      ...
      if (response.data) {
        ...
        const updatedSteps = meeting?.steps?.filter((step) => step.id !== id);
        ...
        // Attempt to reorder the steps, but do not block further execution if it fails
        try {
          await axios.post(
        ...
        // Ensure refreshMeeting is always called, regardless of reorder success
  ...
  useEffect(() => {
    if (id || step) {
      // If editing, select the step's current order
      setSelectedOrder(step?.order_no);
    } else {
      // If adding a new step, auto-select the latest order
      setSelectedOrder((meeting?.steps?.length || 0) + 1);
    }
  }, [meeting?.steps, step, id]); // Update when steps or editing step
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    padding: isMobileView ? "12px" : "12px 16px",
                    display: "flex",
                    alignItems: "center",
                  }}
                >
                ...
                {(!isMobileView || showMetadata) && (
                  <>
                    ...
                    >
                      <div className="p-0 timecard stepinfo" style={{ marginLeft: 0, width: "100%" }}>
                              ...
                              <div className="Editor-custom-dropdown" style={{ width: "100%", position: "relative" }}>
                                {meeting?.type === "Newsletter" || meeting?.type === "Social Media Newsletter" || meeting?.type === "AI Social Media Newsletter" ? (
                                  <>
                                    ...
                                    <span style={{ fontSize: "12px", color: "#344054", fontWeight: "500" }}>
                                  ...
                                  <>
                                    ...
                                    >
                                      <div className="d-flex align-items-center gap-2">
                                        ...
                                        {getLabelForParticipant(user?.email || creator?.email) &&
                                          getStatusLabel(getLabelForParticipant(user?.email || creator?.email))}
                                      </div>
                                      <ExpandIcon />
                                    </div>
                                    {StepPresentor && (
                                              ...
                                              .map((item, index) => {
                                                return item.first_name === null &&
                                                  ...
                                                  <li key={index} disabled>
                                                    {t("meeting.newMeeting.No Guest Available")}
                                                  </li>
                                                ) : (
                                                  <li
                                                    key={index}
                ...
                <div
                  className="col-6 col-sm-4 col-lg-2 editor-header"
                    ...
                    >
                      <div className="p-0 timecard stepinfo" style={{ marginLeft: 0, width: "100%" }}>
                        {!window.location.href.includes("/meetingDetail") && (
                          <div className="p-0 timecard" style={{ width: "100%" }}>
              ...
              <div
                ...
                style={{
                  background: "#F2F4FB",
                  height: isMobileView ? "auto" : "90vh",
                  minHeight: isMobileView ? "auto" : "90vh",
                  overflowY: "auto",
                  padding: 0,
                }}
              >
                ...
                <div>
                  <div className="row">
                    {meeting?.type === "AI Social Media Newsletter" || modalType === "Publication" || meeting?.type === "AI Instruction" ? (
                      <>
                        <div
                          className={`${meeting?.type === "AI Instruction" ? "col-md-12" : "col-md-7"}`}
                          style={
                            meeting?.type === "AI Social Media Newsletter"
                             ? {
                        ...
                        {meeting?.type !== "AI Instruction" && <div className="col-md-5 p-4" style={{ backgroundColor: "#F2F4FB" }}>
                          {!isUpload ? (
                            <>
                              ...
                              {stepMedia.length > 0 && (
                                ...
                                >
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                                    <p style={{ fontSize: "12px", fontWeight: "600", color: "#344054", margin: 0 }}>
                                    ...
                                    {/* <div>
                                      ...
                                      <Button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          document.getElementById("add-extra-media").click();
                                        }}
                                        disabled={isUploadingMedia}
                                        style={{ borderRadius: "20px", fontSize: "12px", padding: "2px 10px" }}
                                      >
                                        {isUploadingMedia ? "Uploading..." : "+ Add"}
                                      </Button>
                                    </div> */}
                                  </div>
      ...
      <Modal
        show={mediaToDelete !== null}
        onHide={() => setMediaToDelete(null)}
        centered
      >
        <Modal.Body className="text-center p-4">
          <div className="mb-3">
            <div style={{
              ...
              background: "#FEE4E2", color: "#D92D20", display: "flex",
          ...
          <h5 style={{ fontWeight: "600", color: "#101828", marginBottom: "8px" }}>
            Supprimer ce média ?
          </h5>
          <p style={{ color: "#475467", fontSize: "14px", marginBottom: "24px" }}>
            {mediaToDelete?.original_name} sera définitivement supprimé.
          </p>
          <div className="d-flex justify-content-center gap-3">
            <Button
              variant="light"
              onClick={() => setMediaToDelete(null)}
              style={{ flex: 1, border: "1px solid #D0D5DD", color: "#344054", fontWeight: "600" }}
            >
              Annuler
            </Button>
            <Button
              variant="danger"
              onClick={() => {
                confirmDeleteMedia(mediaToDelete.id);
                setMediaToDelete(null);
              }}
              style={{ flex: 1, background: "#D92D20", border: "none", fontWeight: "600" }}
            >
...
export default StepChartUpcoming;
